...
| **Syntax highlighting**      | language-labelled tabs, expand/collapse, optional language detection for unlabelled blocks through `@ai-markdown/code-language-detector` (in `@ai-markdown/react-mantine`)                                                                                                                                                         |
...
| [`@ai-markdown/code-language-detector`](./packages/code-language-detector) | Streaming-aware language detection for unlabelled code blocks, and fence language names mapped to Shiki or highlight.js names. | [![@ai-markdown/code-language-detector latest](https://img.shields.io/npm/v/@ai-markdown/code-language-detector/latest?label=npm%20latest&color=blue)](https://www.npmjs.com/package/@ai-markdown/code-language-detector?activeTab=versions) | [![@ai-markdown/code-language-detector monthly downloads](https://img.shields.io/npm/dm/@ai-markdown/code-language-detector?label=downloads%2Fmonth&color=blue)](https://www.npmjs.com/package/@ai-markdown/code-language-detector) |

The five framework/shared packages track `latest`; the independently versioned highlight plugin and code language detector also track `latest`. Download counts are package-wide monthly totals across all versions, not counts for the displayed tag or unique applications. Core and engine are normally installed transitively by an adapter, and the code language detector by the Mantine integration.
...
| `highlight.js`            | `react-mantine` (optional — highlight.js adapter) | `^11.11.2`             |
...
| `codeBlock.autoDetectUnknownLanguage` | `boolean`               | `true`        | Detect the language of unlabelled blocks with `@ai-markdown/code-language-detector`; a block the detector abstains on stays plaintext labelled "unknown"; `false` keeps every unlabelled block plaintext                                                                                                          |
| `codeBlock.languageFormat`            | `MantineLanguageFormat` | `HighlightJs` | Names languages are handed to the highlighter in: `HighlightJs` (`'highlight-js'`) or `Shiki` (`'shiki'`), matching the adapter in `CodeHighlightAdapterProvider`. Applies to fence languages and detected ones alike (` ```objc ` reaches highlight.js as `objectivec`); the tab label keeps the name as written |
...
`preloadMantineCodeAssets()` is also exported by Mantine. It starts the lazy Mermaid import ahead of first use; language detection is synchronous and has nothing to preload. It does not replace the highlight adapter provider or stylesheet imports, and module preloading does not eliminate diagram rendering cost.

## Architecture

`engine` owns parsing and registry primitives; `core` owns framework-independent sessions, plans and coordination. React and Vue depend on both. Mantine composes the React adapter through an exact-version peer dependency. The standalone highlight plugin is a dependency of engine and has its own version; the code language detector is a dependency of the Mantine integration and also has its own version.
...
Public packages live in `packages/{engine,core,react,vue,react-mantine,remark-mark-highlight,code-language-detector}`. `apps/storybook-*`, `tooling/storybook-kit`, `corpus`, `benchmarks/*` and `prototypes/*` are private workspaces. `packages/react/plugins` is a React export subpath, not another published package. See the [development commands](https://ai-markdown.github.io/docs/guides/development-commands/) for the full command map.
...
- The package and version (`@ai-markdown/react@3.2.2` …)
