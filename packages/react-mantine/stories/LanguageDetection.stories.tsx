    ...
    docs: {
      description: {
        component: [
          ...
          '',
          'The Mantine package runs it for fences with no info string when',
          '`codeBlock.autoDetectUnknownLanguage` is `true` (off by default), and uses its',
          'name normalizers for every fence, labelled or not, to translate the name into',
          "the highlighter's spelling.",
          '',
          '```tsx',
          'const AUTODETECT = { autoDetectUnknownLanguage: true }; // module constant',
          '<MantineAIMarkdown content={md} codeBlock={AUTODETECT} />',
          '```',
          '',
