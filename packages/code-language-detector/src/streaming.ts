...
/** How many trailing characters of the last checked input a tail check compares; see the class comment. */
const TAIL_CHECK_CHARS = 64;
/**
 * A tail check runs once the input has grown past the last checked input by
 * this many characters, or by {@link TAIL_CHECK_GROWTH_DIVISOR} of its length
 * when that is more.
 */
const TAIL_CHECK_MIN_GROWTH = 256;
const TAIL_CHECK_GROWTH_DIVISOR = 32;
 ...
 * different text.
 *
 * Looking at the content is never constant work: a caller that builds the text
 * with `+=` passes a V8 cons string, and reading any character of it flattens
 * (copies) the whole string. Doing that on every call would make a stream
 * quadratic (a 200 KB block fed in 4-character steps took seconds instead of
 * milliseconds), so content is looked at on a geometric schedule instead:
 *   - a **tail check** once the text has grown by 256 characters or 1/32 of its
 *     length since the last check, comparing the last 64 characters of the last
 *     checked input at the same offset, so a replacement is noticed within that
 *     much growth unless it repeats those characters exactly;
 *   - the whole text at the growth checkpoints below, when the input is not
 *     longer than the followed text, and in `finalize`, which also catches that
 *     coincidence.
 * Call `reset()` when you know the block was replaced.
 *
...
export class StreamingLanguageDetector {
  ...
  #text = '';
  /** The last input whose continuity was checked, by a tail check, a checkpoint or `finalize` */
  #checkedInput = '';
  #locked = false;
  #finalized = false;
  ...
  update(code: string): LanguageDetectionResult {
    if (this.#finalized) {
      ...
      this.reset();
    } else if (this.#dueForTailCheck(code)) {
      // Longer than the followed text but not a continuation of the last
      // checked input: the same replacement, caught before the next checkpoint.
      if (this.#keepsTailOfCheckedInput(code)) this.#checkedInput = code;
      else this.reset();
    }
  ...
  /** Whether `code` has grown far enough past the last checked input for another tail check */
  #dueForTailCheck(code: string): boolean {
    const checked = this.#checkedInput.length;
    return code.length - checked >= Math.max(TAIL_CHECK_MIN_GROWTH, checked / TAIL_CHECK_GROWTH_DIVISOR);
  }

  /**
   * Whether the last {@link TAIL_CHECK_CHARS} characters of the last checked
   * input sit at the same offset in `code`. A necessary condition for `code`
   * extending that input, not a sufficient one; the checkpoints do the exact
   * comparison.
   */
  #keepsTailOfCheckedInput(code: string): boolean {
    const checked = this.#checkedInput;
    const start = Math.max(0, checked.length - TAIL_CHECK_CHARS);
    return code.startsWith(checked.slice(start), start);
  }
