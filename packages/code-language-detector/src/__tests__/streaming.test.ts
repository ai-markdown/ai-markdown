...
describe('StreamingLanguageDetector follows one growing text', () => {
  ...
  it('a longer replacement between checkpoints is caught on the call where it arrives', () => {
    const streaming = new StreamingLanguageDetector();
    expect(streaming.update(rust).language).toBe('rust');
    // Grow past the checkpoint text without reaching the next checkpoint.
    const grown = `${rust}\n// helpers follow`;
    expect(streaming.update(grown).language).toBe('rust');

    // Longer than the checkpoint text, shorter than the next checkpoint, and not a continuation of the last input.
    const python =
      'import os\n\ndef load(path):\n    with open(path) as f:\n        return f.read()\n\n\ndef exists(path):\n    return os.path.exists(path)\n';
    expect(python.length).toBeGreaterThan(rust.length);
    expect(python.length).toBeLessThan(rust.length + 80);
    expect(streaming.update(python).language).toBe('python');
  });
