...
**Following one text.** An input that does not extend the followed text (`startsWith` fails) is a different text and resets the detector. Comparing the whole text is linear in its length, so it is not done on every call (per token, a 200 KB block would take seconds instead of milliseconds). Every `update` instead checks, in constant work, that the last 64 characters of the previous input are still at the same offset, so a replacement longer than the followed text, such as a regenerated block, resets the detector on the call where it arrives. The whole text is compared when the input is not longer than the followed text, at growth checkpoints and in `finalize`; a replacement that happens to repeat those 64 characters at the same offset is caught there. The stream as a whole still costs linear time. After `finalize`, the identical text returns the cached result, an extension resumes streaming with the current verdict, and anything else resets.
...
- **One detector per code block.** The detector follows a single growing text. Reusing it for the next fence works (a non-extension resets it): the per-call tail check catches a longer replacement on the call where it arrives, and the whole-text comparison at the next growth checkpoint or at `finalize` catches the rare replacement that repeats the last 64 characters of the previous input at the same offset. If you know a block was regenerated or replaced, call `reset()`, which does not depend on either check.
