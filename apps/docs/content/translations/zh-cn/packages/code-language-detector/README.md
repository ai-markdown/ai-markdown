...
**跟踪同一段文本。** 如果输入不是所跟踪文本的延伸（`startsWith` 不成立），它就是另一段文本，检测器会重置。比较整段文本的耗时与文本长度成线性关系，因此不会在每次调用时都执行（如果按 token 调用，一个 200 KB 的代码块会耗时数秒而不是数毫秒）。每次 `update` 改为以常数开销检查上一次输入的最后 64 个字符是否仍位于相同偏移处，因此比所跟踪文本更长的替换文本（例如重新生成的代码块）会在它到达的那次调用中就重置检测器。整段文本的比较在以下时机执行：输入不长于所跟踪的文本时、到达增长检查点时，以及 `finalize` 中；替换文本恰好在相同偏移处重复了这 64 个字符的情况，会在这些时机被发现。整个流的总开销仍然是线性的。`finalize` 之后，完全相同的文本返回缓存结果，延伸文本以当前判定恢复流式检测，其他情况一律重置。
...
- **每个代码块使用一个检测器。** 检测器跟踪的是单段不断增长的文本。复用它处理下一个 fence 是可行的（非延伸文本会触发重置）：每次调用的尾部检查会在更长的替换文本到达时立即发现它；替换文本在相同偏移处重复了上一次输入最后 64 个字符的少见情况，则由下一个增长检查点或 `finalize` 中的整段比较发现。如果你知道某个代码块已被重新生成或替换，请调用 `reset()`，它不依赖上述任何一种检查。
