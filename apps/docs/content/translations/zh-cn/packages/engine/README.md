...
| 跨片段协调 | `documentRegistry`、`collectDefLabels`、`extractContributions`、`extractDefBodiesFromHast`、`crossChunkUrlSanitize`                                              | `createRegistry()` —— 负责单文档内跨片段脚注编号与链接定义解析的存储中心；`resolveCrossChunkReference()` 对已解析的跨片段链接或图片应用 schema、hash 重定基和 URL 变换，与独立渲染管线的门禁一致；`sanitizeCrossChunkUrl()` 已弃用：它只对裸 URL 执行协议门禁和 URL 变换（不重定基 hash，不处理 `title`/`alt`），不在适配器的渲染路径上，3.x 内保留导出——请改用 `resolveCrossChunkReference()`                        |
