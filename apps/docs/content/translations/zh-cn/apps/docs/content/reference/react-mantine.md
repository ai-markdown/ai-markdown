...
| `autoDetectUnknownLanguage` | `boolean`               | `true`                              | 使用 `@ai-markdown/code-language-detector` 识别未标注语言代码块的语言，在渲染期间同步执行（包括服务端渲染）。检测器放弃判断的代码块保持纯文本，标签为“unknown”；检测器永远不会覆盖显式声明的围栏语言                                                                                                                                                                                    |
