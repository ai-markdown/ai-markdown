...
| `pnpm build`                                  | 构建全部七个公开相关包并执行其分发断言。不包含 Storybook 和基准测试应用。                     |
...
| `pnpm version-packages <version>` | 重写根目录和五个统一版本发布相关包的版本及相关引用。高亮插件与代码语言探测器保持独立版本。该命令不会发布；执行后需同步 lockfile 并进行审查。 |
