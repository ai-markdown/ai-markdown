...
- **Certifier shapes found by the release soak.** An empty marker line (`-` then `  [x] a`) never certifies, and after a root indented code block the marker rules run in micromark's interrupt mode, so `12. [x] done` there is paragraph text whose `[x]` a late definition turns into a link. Both shapes are pinned as splice-equivalence and syntax cases.
...
All 2,768 unit tests pass together with the splice fuzz, the Storybook suites, the packed consumers, the document-lifetime and Vue browser checks, and the full preflight including Firefox and WebKit. Each engine change was reviewed independently by a second agent, whose reproductions became permanent tests (`taskListTaint`, `taskListMathCapability`, `mathCapabilityOracle`, `tagNameBoundary`, `tabIndentAxis`, `tagNamePrefixAxis`).

A fresh release-profile engine soak with seed base `202609700` passed all six legs and **84/84 shards** in 10,874 seconds on commit `22b05e3`. Two earlier campaigns on this candidate did their job: seed `202609405` (fuzz) and seed `202609711` (direction battery) found the two certifier shapes above, and the oracle leg crossed the fully-blind hazard-document limit calibrated on the 3.0.x corpus (8.22% against 8%). That limit was re-derived by the documented method, 12 sweeps of 4,000 runs on the 3.1.0 corpus measuring 5.79–8.14%, and moved to 12% with a per-family attribution readout added to the oracle. The evidence was validated against the tagged commit before publishing.
...
- **Engine API note:** `sanitizeCrossChunkUrl` (engine barrel) now returns `string | null` — `null` for a protocol-blocked URL (render the attribute absent), `''` for a legal empty destination. `@ai-react-markdown/core` pins the engine exactly; direct engine importers see the widened type. (Deprecated since 3.0: the adapters resolve cross-chunk references with `resolveCrossChunkReference`, which also rebases hash links and sanitizes the final element; `sanitizeCrossChunkUrl` stays exported through 3.x.)
